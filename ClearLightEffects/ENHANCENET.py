...
from ClearLightEffects.dataset import ImageFolder
from torchvision import transforms
from torch.utils.data import DataLoader
from ClearLightEffects.networks import *
...
class ENHANCENET(object):
    ...
    def build_model(self):
        ...
        self.genA2B = ResnetGenerator(input_nc=3, output_nc=3, ngf=self.ch, n_blocks=self.n_res,
                                      img_size=self.img_size).to(self.device)
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            ...
            for i in range(-1, 0, 1):
                ...
                # path changed: ./output -> direct path
                path_fakeB = os.path.join('ClearLightEffects/output')
                if not os.path.exists(path_fakeB):
                    os.makedirs(path_fakeB)

                self.gt_list = [os.path.splitext(f)[0] for f in os.listdir(os.path.join(self.datasetpath)) if
                                f.endswith(self.im_suf_A)]
                for n, img_name in enumerate(self.gt_list):
                    ...
                    del img, real_A, fake_A2B, A_real, B_fake
                    gc.collect()
